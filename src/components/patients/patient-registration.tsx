...
import { PatientRegistrationRequest, BatchPatientResponse, PatientManagementApi } from "@/generated";
...
const PatientRegistration: React.FC = () => {
  const [formData, setFormData] = useState<PatientRegistrationRequest>(mockPatient);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (formData.insurance?.hasInsurance) {
      if (!formData.insurance.insuranceType || !formData.insurance.schemeId || !formData.insurance.policyHolderName) {
        toast({
      ...
      if ((response.status === 200 || response.status === 201) && response.data) {
        if (response.data.successCount === 1) {
          ...
          toast({
            ...
            description: `Patient ID: ${patientResponse?.patientId || ''} - ${formData.firstName} ${formData.lastName} has been saved successfully.`,
          });
          ...
          const errors = response.data.failedRegistrations.map(failure => 
